    ...
    <section class="bg-cream text-charcoal min-h-screen font-sans leading-normal overflow-x-hidden lg:overflow-auto">
      <main class="flex-1 md:p-0 lg:pt-8 lg:px-8 md:ml-24 flex flex-col">
        <section class="bg-cream-lighter p-4 shadow">
          <div class="md:flex">
            <h2 class="md:w-1/3 uppercase tracking-wide text-sm sm:text-lg mb-6">Edit recipe</h2>
          </div>
          ...
          <div class="md:flex mb-6 ">
            <div class="md:flex-1 px-3 text-center md:text-right">
              ...
              <button class="bg-teal-500 hover:bg-blue-700 text-white text-xs font-bold py-1 px-2 rounded-full" @click="deleteIngredient">Delete Ingredient</button>
            </div>
          </div>
          ...
          <div class="md:flex mb-6 ">
            <div class="md:flex-1 px-3 text-center md:text-right">
              ...
              <button class="bg-teal-500 hover:bg-blue-700 text-white text-xs font-bold py-1 px-2 rounded-full" @click="deleteStep">Delete Step</button>
            </div>
          </div>
          ...
          <div class="md:flex mb-6">
            <div class="md:flex-1 px-3 text-center md:text-right justify-center">
              <input type="hidden" name="sponsor" value="0">
              <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full" @click="addRecipe">Update Recipe</button>
            </div>
          </div>
        </section>
      </main>
...
export default {
  ...
  created() {
    this.Steps = this.$route.params.recipeList.Steps;
    this.ingredients = this.$route.params.recipeList.ingredients;
    this.imageURL = this.$route.params.recipeList.imageURL;
    this.name = this.$route.params.recipeList.name;
    this.description = this.$route.params.recipeList.description;
    this.recipe = this.$route.params.recipeList;
  },
  methods: {
    addStep() {
